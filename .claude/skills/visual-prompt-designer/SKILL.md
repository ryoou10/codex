...
This skill is the design-translation layer. It does not replace image generation tooling. When the user asks to create or edit a raster image, hand the improved prompt to the session's image-generation tool or MCP after this prompt pass when one is configured. Claude Code has no built-in image-generation tool; when none is available, the improved prompt and handoff spec are the deliverable.

## Combined Generation Flow
...
2. Convert the improved prompt into a generation handoff spec using the shared schema below.
3. Generate or edit through the available image-generation tool.
...
6. Report the final generation mode, saved path when project-bound, and the final prompt or handoff spec used.
...
6. If the user wants an actual generated image, hand off the improved prompt for generation:
   - classify the use case, usually `illustration-story`, `stylized-concept`, `historical-scene`, or `photorealistic-natural`
...
Keep `Diagnosis` and `Design Direction` concise. The `Improved Prompt` should be directly usable in an image-generation tool. Include `Generation Handoff` only when the next action is to generate or edit an image. Include `Result Check` only after an image has been generated or inspected.
...
When handing off for generation, compress the improved prompt into this shared prompt schema:

```text
Use case: <use-case slug such as stylized-concept>
...
If the user only asked for prompt improvement, stop after the written output. If the user asked to generate, use the session's image-generation tool when one is configured; otherwise deliver the handoff spec and state that no image was generated.

## Character/Worldbuilding Generation Defaults
...
- Transparency: prefer a chroma-key-friendly solid background unless the generation tool natively supports transparency.
