...
Use this skill as the creative-director layer before image generation. Its single purpose is to inspect provided visual inputs, merge them with the user's optional conditions, fill missing creative choices coherently, and hand a production-ready prompt to an image-generation tool when one is available.

When the user expects a bitmap result, check whether the session has an image-generation tool or MCP configured. If one exists, generate through it. Claude Code has no built-in image-generation tool, so when none is configured, deliver the finalized spec as the output, state clearly that no image was generated, and ask the user how to proceed.

## Workflow

1. Locate every image input:
   - If the user provides local file paths, inspect each image with the Read tool before using it.
   ...
   - If iCloud paths are visible to the shell but the Read tool cannot open them, copy the selected image non-destructively to a temp directory and read the copy. If that still fails, verify with `sips -g pixelWidth -g pixelHeight` and ask the user for an attachment only if visual inspection is impossible.
...
6. Build a generation handoff spec and then generate when a tool is available:
...
## Generation Handoff Format

Use this format before generation or editing:

```text
Use case: <use-case slug such as stylized-concept or illustration-story>
Mode: <generate | edit>
...
Use `scripts/list_icloud_images.py` (macOS only; relies on the iCloud sync path and `sips`) when the user points to iPhone Files > iCloud Drive > image files and does not name a specific file:
...
When generating an image, include the final prompt or handoff spec, the saved path for project-bound assets, and any iteration performed. When no image-generation tool is available, deliver the spec and say so explicitly.
