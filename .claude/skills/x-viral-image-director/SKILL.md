...
1. Gather references:
   ...
   - If the Read tool cannot inspect local or attachment paths, verify files with `file`, `sips`, and `scripts/analyze_reference_set.py`; continue from measurable evidence when direct visual inspection is unavailable.
2. Analyze the image set:
   - Run `scripts/analyze_reference_set.py --icloud --attachment <path>` (requires Pillow) when an attachment exists.
...
4. Build an image-generation prompt:
...
5. Generate through the session's image-generation tool or MCP when one is configured. Claude Code has no built-in image-generation tool; when none is available, deliver the finalized prompt spec and state that no image was generated.
6. Verify and report:
   - Confirm the generated file's saved path unless the user requested a project destination.
   - Report reference roles, the selected X strategy, and the final prompt summary.

## Handoff Prompt Shape

Use this structure before generating:
